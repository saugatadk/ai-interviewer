    ...
    <Card className='mb-6 overflow-hidden shadow-md border-emerald-100'>
      ...
      <CardContent className='p-6'>
        <div className='space-y-5'>
          {Object.entries(scorecard).map(([skill, data]) => {
            // Provide defaults in case data or fields are missing
            const score = data?.score ?? 0;
            const commentary = data?.commentary ?? "No commentary provided.";
              ...
              <div key={skill} className='space-y-3'>
                <div className='flex justify-between items-center'>
                  ...
                  <div className='flex items-center gap-2'>
                    <span className='bg-gray-100 text-gray-800 text-sm py-0.5 px-2 rounded-lg font-medium'>{score}/10</span>
                  </div>
                </div>
